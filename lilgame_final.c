...
void game1()
{
	...
	{
		system("cls");
		printf("pergunta 1: quando descobriram o brasil?\n\n");
		printf("a - 1692\n");
		printf("b - 1822\n");
		printf("c - 1500\n");
		printf("d - 1592\n\n");
	...
	{
		system("cls");
		printf("pergunta 2: o que e um tomate, cientificamente?\n\n");
		printf("a - planta\n");
		printf("b - fruta\n");
		printf("c - carne\n");
		printf("d - vegetal\n\n");
	...
	{
		system("cls");
		printf("voce errou. (correta: a, sua: %c)\n", ugs);
	}
	printf("voce acertou %d perguntas ate agora", score);
	ugs = 'p';
	while(ugs != 'c')
	...
	{
		system("cls");
		printf("pergunta 3: qual pokemon, na pokedex nacional, tem numero #025?\n\n");
		printf("a - rattata\n");
		printf("b - pikachu\n");
		printf("c - caterpie\n");
		printf("d - zubat\n\n");
	...
	{
		system("cls");
		printf("voce errou. (correta: b, sua: %c)\n", ugs);
	}
	printf("voce acertou %d perguntas ate agora", score);
	ugs = 'p';
	while(ugs != 'c')
	...
	{
		system("cls");
		printf("pergunta 4: a musica reality check through the skull, da comunidade de beat saber, e qual dificuldade?\n\n");
		printf("a - expert+\n");
		printf("b - expert\n");
		printf("c - hard\n");
		printf("d - normal\n\n");
	...
	{
		system("cls");
		printf("pergunta 5: qual o pib do canada (em milhao de usd)\n\n");
		printf("a - 2.509.421\n");
		printf("b - 3.558.562\n");
		printf("c - 2.701.544\n");
		printf("d - 2.420.837\n\n");
	...
	{
		system("cls");
		printf("voce errou. (correta: d, sua: %c)\n", ugs);
	}
	printf("voce acertou %d perguntas ate agora", score);
	ugs = 'p';
	while(ugs != 'c')
...
void game3()
{
	...
	int p1t = 0, p2t = 0; // choices for each player, as int
	int p1a = 0, p2a = 0; // attack choice
	int turns = 0; //again, why not
	int turnof = (rand() % 2) + 1; // whose turn it is
	int loopy = 1;
	int loopery = 1;
	int loopenery = 1;
	int p1g1s = 1, p1g2s = 1, p2g1s = 1, p2g2s = 1; // gousma status (fnt/active)
	int p1g1 = 1, p1g2 = 1, p2g1 = 1, p2g2 = 1; // fury count
	int divide; // guess what this one's for
	int winner = 0;
	
	//stuff:
	printf("primeiro turno vai ao player %d\n", turnof);
	while(ugs != 'c')
	{
		printf("\ndigite c para continuar: ");
		ugs = getchar();
	...
	while(loopy == 1)
	{
		...
		switch(turnof)
		{
			case 1:
				...
				printf("\nlado do adversario\n"); // enemy stats
				printf("gousma 1: %dF ", p2g1);
				if(p2g1s == 0)
				{
					printf("(DSTR) ");
				}
				printf("/ gousma 2: %dF",p2g2);
				if(p2g2s == 0)
				{
					printf(" (DSTR)\n\n");
				}
				...
				printf("seu lado\n"); // your stats
				printf("gousma 1: %dF ", p1g1);
				if(p1g1s == 0)
				{
					printf("(DSTR) ");
				}
				printf("/ gousma 2: %dF",p1g2);
				if(p1g2s == 0)
				{
					printf(" (DSTR)\n\n");
				}
				...
				while(loopery == 1)
				{
					...
					{
						ugs = 'p';
						printf("\nescolha sua acao (1 = atacar, 2 = dividir): ");
						ugs = getchar();
						p1t = (int)ugs - 48;
					}
					switch(p1t)
					{
						case 1:
							...
							while(loopenery == 1)
							{
								...
								{
									ugs = 'p';
									printf("\nescolha a gousma que vai atacar: ");
									ugs = getchar();
									p1a = (int)ugs - 48;
								}
								switch(p1a)
								{
									case 1:
										if(p1g1s == 0)
										{
											printf("gousma nao consegue atacar.");
										}
										else
										{
											loopenery = 0;
							...
							while(loopenery == 1)
							{
								...
								{
									ugs = 'p';
									printf("\nescolha a gousma que vai ser atacada: ");
									ugs = getchar();
									p2a = (int)ugs - 48;
								}
								switch(p2a)
								{
									case 1:
										if(p2g1s == 0)
										{
											printf("gousma nao consegue ser atacada.");
										}
										else
										{
											loopenery = 0;
							...
							switch(p1a)
							{
								case 1:
									switch(p2a)
									{
										case 1:
											p2g1 = p2g1 + p1g1;
											printf("ataque completo. (g1 > g1)\n");
											break;
										case 2:
											p2g2 = p2g2 + p1g1;
											printf("ataque completo. (g1 > g2)\n");
											break;
										default:
											printf("\nisso nao deveria acontecer, se vc ta vendo essa msg ent quebrou o programa (p2a-d)\n");
											break;
									}
									break;
								case 2:
									switch(p2a)
									{
										case 1:
											p2g1 = p2g1 + p1g2;
											printf("ataque completo. (g2 > g1)\n");
											break;
										case 2:
											p2g2 = p2g2 + p1g2;
											printf("ataque completo. (g2 > g2)\n");
											break;
										default:
											printf("\nisso nao deveria acontecer, se vc ta vendo essa msg ent quebrou o programa (p2a-d)\n");
											break;
									...
									break;
								default:
									printf("\nisso nao deveria acontecer, se vc ta vendo essa msg ent quebrou o programa (p1a-d)\n");
									break;
							}
							loopery = 0;
							break;
						case 2:
							divide = p1g1 + p1g2;
							if(((divide / 2) == p1g1) && ((divide / 2) == p1g2))
							{
								printf("divisao faria nada.");
							}
							else if(((divide % 2) == 0) && ((divide / 2) < 9))
							{
								p1g1 = divide / 2;
								p1g2 = divide / 2;
								printf("divisao completa. (%d/%d)", p1g1, p1g2);
								loopery = 0;
							}
							else
							{
						...
						break;
					default:
						printf("\nisso nao deveria acontecer, se vc ta vendo essa msg ent quebrou o programa (p1t-d)\n");
						break;
					}
				}
				turns++;
			...
			case 2:
				...
				while(loopery == 1)
				{
					...
					switch(p2t)
					{
						...
						case 2:
							divide = p2g1 + p2g2;
							if(((divide / 2) == p2g1) && ((divide / 2) == p2g2))
							{
								printf("divisao faria nada.");
							}
							else if(((divide % 2) == 0) && ((divide / 2) < 9))
							{
								p2g1 = divide / 2;
								p2g2 = divide / 2;
								printf("divisao completa. (%d/%d)", p2g1, p2g2);
								loopery = 0;
							}
							else
							{
						...
						break;
					default:
						printf("\nisso nao deveria acontecer, se vc ta vendo essa msg ent quebrou o programa (p2t-d)\n");
						break;
					}
				}
				break;
			default:
				printf("\nisso nao deveria acontecer, se vc ta vendo essa msg ent quebrou o programa (turnof-d)\n");
				break;
		}
		// p1 checks
		if((p1g1s == 1) && (p1g1 >= 5))
		{
			p1g1s = 0;
			p1g1 = 0;
			printf("\ngousma 1 de player 1 foi destruida.");
		}
		if((p1g2s == 1) && (p1g2 >= 5))
		{
			p1g2s = 0;
			p1g2 = 0;
			printf("\ngousma 2 de player 1 foi destruida.");
		}
		if((p1g1s == 0) && (p1g1 > 0))
		{
			p1g1s = 1;
			printf("\ngousma 1 de player 1 foi revivida.");
		}
		if((p1g2s == 0) && (p1g2 > 0))
		{
			p1g2s = 1;
			printf("\ngousma 2 de player 1 foi revivida.");
		}
		// p2 checks
		if((p2g1s == 1) && (p2g1 >= 5))
		{
			p2g1s = 0;
			p2g1 = 0;
			printf("\ngousma 1 de player 2 foi destruida.");
		}
		if((p2g2s == 1) && (p2g2 >= 5))
		{
			p2g2s = 0;
			p2g2 = 0;
			printf("\ngousma 2 de player 2 foi destruida.");
		}
		if((p2g1s == 0) && (p2g1 > 0))
		{
			p2g1s = 1;
			printf("\ngousma 1 de player 2 foi revivida.");
		}
		if((p2g2s == 0) && (p2g2 > 0))
		{
			p2g2s = 1;
			printf("\ngousma 2 de player 2 foi revivida.");
		}
		// win checks
		if((p1g1s == 0) && (p1g2s == 0))
		{
			printf("\nplayer 1 nao tem gousmas ativas. player 2 vence!");
			winner = 2;
			loopy = 0;
		}
		if((p2g1s == 0) && (p2g2s == 0))
		{
			printf("\nplayer 2 nao tem gousmas ativas. player 1 vence!");
			winner = 1;
			loopy = 0;
		}
		ugs = 'p';
	...
	system("cls");
	printf("player %d vence. (em %d turnos)", winner, turns);
	ugs = 'p';
	while(ugs != 'f')
		{
			printf("\ndigite f para finalizar: ");
